...
from chatzzk_core.constants import StreamAtmosphere
...
class StreamSegmentAnalysisInput(BaseModel):
    ...
    @classmethod
    def from_analysis_params(cls, params: StreamSegmentAnalysisParams) -> ["StreamSegmentAnalysisInput"]:
...
# llm generation을 수행할 때 필요한 스키마 정의할 때 고려할 것
# task, 입출력
...
class SummaryGenerationInput(BaseModel):
    ...
    channel_metadata: ChannelMetadata
    previous_summary: str | None = None
    broadcast_logs: str | None = None  # 없을 수 있으나 없으면 llm api를 호출하지말고 기본값으로 처리해야함
