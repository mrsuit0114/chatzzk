...
class LLMClient(ABC):
    ...
    @abstractmethod
    async def async_complete(self, api_key: str, **kwargs):
        pass
    ...
    def show_prompt(self, **kwargs):
        ...
        user_prompt = self.prompt_builder._build_user_prompt(**kwargs)
...
class ShortTermSummaryLLMClient(LLMClient):
    ...
    async def async_complete(self, api_key: str, **kwargs) -> str:
...
class GeneralChoiceLLMClient(LLMClient):
    def __init__(self, config: dict, proxy_url: str):
        ...
        self.choice_num = config["general_choice"]["choice_num"]

    async def async_complete(self, api_key: str, **msg_kwargs) -> list[str]:
        messages = self._build_messages(**msg_kwargs)
        try:
            response = await acompletion(
                model=self.task_type,
                messages=messages,
    ...
    def complete(self, api_key: str, **kwargs) -> list[str]:
        messages = self._build_messages(**kwargs)
