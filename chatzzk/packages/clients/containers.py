...
class ClientsContainer(containers.DeclarativeContainer):
    ...
    base_http_config = providers.Callable(
        BaseHttpConfig.model_validate,
        config.api.base_http,
    )

    chzzk_api_config = providers.Callable(
        ChzzkApiConfig.model_validate,
        config.api.chzzk_api,
    )
    ...
    base_http_client = providers.Singleton(
        BaseHttpClient,
        config=base_http_config,
    )

    chzzk_api_client = providers.Factory(
        ChzzkApiClient,
        config=chzzk_api_config,
