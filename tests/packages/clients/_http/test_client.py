...
@pytest.fixture
def mock_config() -> ApiClientConfig:
    """테스트용 기본 설정 객체를 생성합니다."""
...
@pytest.mark.asyncio
async def test_successful_json_request(mock_config):
    """성공적인 JSON GET 요청을 테스트합니다."""
    async with BaseHttpClient(mock_config) as client:
...
@pytest.mark.asyncio
async def test_successful_json_with_content_key(mock_config):
...
@pytest.mark.asyncio
async def test_successful_text_request(mock_config):
...
@pytest.mark.asyncio
async def test_retry_on_failure_then_succeed(mock_config):
...
@pytest.mark.asyncio
async def test_retry_exhausted(mock_config):
...
@pytest.mark.asyncio
async def test_context_manager_closes_session(mock_config):
    """컨텍스트 관리자(async with)가 세션을 올바르게 닫는지 테스트합니다."""
    client = BaseHttpClient(mock_config)
